  ...
  templateShowcase = {
    ...
    updateTitle: (title: string) => {
      if (!this.worksData.templateShowcaseInfo) {
        this.worksData.templateShowcaseInfo = this.templateShowcase.getDefaultInfo();
      }
...
app.get('/api/marketplace/templates', async (req, res) => {
  ...
  const result = templates.map(t => {
    ...
    return {
      ...
      coverImage: worksData.templateShowcaseInfo.previewImages.find(img => img.isCover)
    };
  });
      ...
      <div className='grid grid-cols-3 gap-2'>
        {images.map(img => (
          <ImageCard
            ...
            onRemove={() => worksStore.templateShowcase.removePreviewImage(img.id)}
          />
        ))}
      </div>
    </div>
...
import { useGridContext } from '../../../comp/provider';
import { getImgInfo2 } from '../../../shared/utils';
import { random } from '@mk/utils';  // 导入现有的 random 函数
