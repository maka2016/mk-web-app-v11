...
export async function tokenToUid(token: string): Promise<{
  ...
  log.info({ token, v11: process.env.APIV11 }, 'tokenToUid');
  ...
  log.info({ url, token }, 'tokenToUid-APIV10');
  ...
  try {
    ...
    log.info({ uid: data.user.uid, appid: data.user.appid }, 'tokenToUid-APIV10');
