...
export const worksAIVectorRouter = router({
  ...
  vectorizeWorks: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const analyzeResult = await analyzeTemplateForVector(works.work_data, works.detail.title ?? '');
      ...
      if (!embeddingText.trim()) {
        const elements = AIWorksSDK2.extractTemplateTextElements(works.work_data);
