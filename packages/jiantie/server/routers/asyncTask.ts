...
// 任务类型枚举 Schema
const asyncTaskTypeSchema = z.enum([
...
// 最终生成的 GIF 固定尺寸，遇到 GIF 直接返回该尺寸，无需解析
const GIF_FIXED_WIDTH = 540;
const GIF_FIXED_HEIGHT = 960;
...
async function getImageDimensions(url: string): Promise<{ width: number; height: number } | null> {
  try {
    ...
    // GIF 固定为 270x480，不采用解析出的尺寸
    if (result.type === 'gif') {
      return { width: GIF_FIXED_WIDTH, height: GIF_FIXED_HEIGHT };
    }
...
// 处理按模版 ID 列表批量向量化任务
async function processTemplateBatchVectorizeByIdsTask(prisma: any, task: any) {
  const inputData = task.input_data as { template_ids: string[]; skip_ai_analyze?: boolean };
  const templateIds = inputData.template_ids ?? [];
  const skipAiAnalyze = inputData.skip_ai_analyze ?? false;
  return vectorizeTemplatesBatch(prisma, templateIds, skipAiAnalyze);
}

// 处理按模版创建时间范围批量向量化任务
async function processTemplateBatchVectorizeByDateTask(prisma: any, task: any) {
  const inputData = task.input_data as {
    start_date: string;
    end_date: string;
    skip_ai_analyze?: boolean;
  };
  ...
  if (templateIds.length > BATCH_VECTORIZE_MAX_COUNT) {
    throw new Error(
      `范围内模版数量过多（${templateIds.length}），请缩小时间范围，单次最多处理 ${BATCH_VECTORIZE_MAX_COUNT} 个`
    );
  }
...
export const asyncTaskRouter = router({
  ...
  processTask: publicProcedure.input(z.object({ id: z.string() })).mutation(async ({ ctx, input }) => {
    ...
    } else if (task.task_type === 'template_batch_vectorize_by_ids') {
      total = inputData.template_ids?.length || 0;
    } else if (task.task_type === 'template_batch_vectorize_by_date') {
      const startDate = new Date(inputData.start_date);
    ...
    try {
      ...
      if (task.task_type === 'batch_generate_covers') {
        ...
        const result = await processTemplateBatchVectorizeByIdsTask(ctx.prisma, task);
        ...
        const result = await processTemplateBatchVectorizeByDateTask(ctx.prisma, task);
