  ...
  findManyInternal: publicProcedure
    .input(
      z.object({
        ...
        create_time_from: z.string().optional(), // 创建时间起（ISO 日期字符串）
        create_time_to: z.string().optional(), // 创建时间止（ISO 日期字符串）
        skip: z.number().optional(),
        take: z.number().optional(),
      })
    )
    .query(async ({ ctx, input }) => {
      ...
      // 创建时间范围
      if (input.create_time_from || input.create_time_to) {
        where.create_time = {};
        if (input.create_time_from) {
          where.create_time.gte = new Date(input.create_time_from);
        }
        if (input.create_time_to) {
          const d = new Date(input.create_time_to);
          d.setHours(23, 59, 59, 999);
          where.create_time.lte = d;
        }
      }
      if (input.is_folder !== undefined) where.is_folder = input.is_folder;
      if (input.folder_id !== undefined) where.folder_id = input.folder_id;
  ...
  countByUid: publicProcedure
    .input(
      z.object({
        ...
        version_gte: z.number().optional(), // 版本大于等于
        is_paid: z.enum(['all', 'paid', 'unpaid']).optional().default('all'), // 支付状态：全部、已付费、未付费
        create_time_from: z.string().optional(),
        create_time_to: z.string().optional(),
      })
    )
    .query(async ({ ctx, input }) => {
      const where: any = {};
