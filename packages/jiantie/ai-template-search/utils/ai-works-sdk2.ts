...
export class AIWorksSDK2 {
    ...
    const traverse = (
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const layout: 'column' | 'row' =
          row.style?.flexDirection === 'row' ? 'row' : 'column';
        const gap =
          typeof row.style?.gap === 'number'
            ? `${row.style.gap}px`
            : (row.style?.gap as string | undefined);
  ...
  static async executeContentAndLayout(
    ...
    for (const add of analysisResult.contentPlan.add) {
      try {
        ...
        // 容错：AI 常漏写 block 下第一层索引（block 通常只有 1 个子节点 grid_root），导致 [0,2,...] 应为 [0,0,2,...]
  ...
  static async generateWithAgentWorkflow(
    ...
    const worksStore = typeof options === 'string' ? (options as unknown as { worksStore: WorksStore }).worksStore : options.worksStore;
      ...
      try {
        ...
        if (status === 'success') {
          body.final_snapshot = {
            ...
            final_layout_structure: this.serializeGridsDataStructure(
              worksStore.worksData.gridProps?.gridsData || []
            ),
          };
        }
    ...
    while (iteration < maxIterations) {
      ...
      try {
        ...
        const validationResult = await this.validateFinalWorks(
          userInput,
          worksStore.worksData,
          validateCtx
        );
