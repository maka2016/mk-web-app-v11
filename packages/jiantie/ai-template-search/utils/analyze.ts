...
import { AIWorksSDK2 } from './ai-works-sdk2';
...
export function extractTemplateText(worksData: IWorksData): string {
  const textElements = AIWorksSDK2.extractTemplateTextElements(worksData);
...
function parseAIResponse(text: string): AIVectorMeta {
  ...
  return {
    ...
    industryTags: Array.isArray(parsed.industryTags)
      ? parsed.industryTags.filter((t: unknown) => typeof t === 'string')
      : [],
    styleTags: Array.isArray(parsed.styleTags) ? parsed.styleTags.filter((t: unknown) => typeof t === 'string') : [],
    audienceTags: Array.isArray(parsed.audienceTags)
      ? parsed.audienceTags.filter((t: unknown) => typeof t === 'string')
      : [],
 ...
 * @returns 分析结果
 */
export async function analyzeTemplateForVector(worksData: IWorksData, templateTitle: string): Promise<AnalyzeResult> {
  // 检查 API Key
