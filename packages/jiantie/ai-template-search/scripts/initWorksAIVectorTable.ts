// 初始化用户作品 AI 向量表脚本
// 在主数据库中创建 works_ai_vectors 表及其索引
// 复用 DATABASE_URL 数据库
//
// 使用方法：
//   cd packages/jiantie
//   pnpm init:works-ai-vector-table
// 或者：
//   tsx ai-template-search/scripts/initWorksAIVectorTable.ts
...
async function main() {
  try {
    ...
    const sqlPath = join(__dirname, 'createWorksAIVectorTable.sql');
    ...
    // 先执行表创建语句
    const tableCreateStatements = statements.filter(s => s.toUpperCase().includes('CREATE TABLE'));

    // 再执行索引创建语句
    const indexCreateStatements = statements.filter(s => s.toUpperCase().includes('CREATE INDEX'));
    ...
    const checkResult = await dbPool.query(`
      SELECT EXISTS (
        ...
        AND table_name = 'works_ai_vectors'
      );
    `);

    if (checkResult.rows[0]?.exists) {
      console.log('✓ 验证：works_ai_vectors 表已存在\n');
    } else {
      throw new Error('表创建失败：验证时未找到 works_ai_vectors 表');
    }
    ...
    const indexResult = await dbPool.query(`
      SELECT indexname
      FROM pg_indexes
      WHERE tablename = 'works_ai_vectors'
      ORDER BY indexname;
    `);
    ...
    const columnsResult = await dbPool.query(`
      ...
      WHERE table_name = 'works_ai_vectors'
