...
export default class APPBridge {
  ...
  static setShareInfo2MiniP(info: {
    title: string;
    imageUrl: string;
    path: string;
  }): void {
          ...
          if (isMakaAppAndroid() || urlParams?.runtime === 'ANDROID') {
            ...
            if (!userInfo.vipInfo) {
              ...
              sessionStorage.setItem(
                'editor_vip',
                userInfo?.vipInfo && userInfo?.vipInfo?.level > 0 ? '1' : '0'
              );
            }
  ...
  static async appCall(
    appParams: appInvokeParamsType,
    jsFn?: (params?: any | null) => void,
    timeout = 1000
  ) {
    if (!APPBridge.judgeIsInApp()) {
      return false;
    ...
    const appInvoke = () => {
      if (
        (this.getRuntime() === 'IOS' || isMakaAppIOS()) &&
        WINDOW?.webkit?.messageHandlers?.APP_INVOKE?.postMessage
      ) {
        ...
        (this.getRuntime() === 'ANDROID' || isMakaAppAndroid()) &&
        WINDOW?.nativeApi?.APP_INVOKE
      ) {
