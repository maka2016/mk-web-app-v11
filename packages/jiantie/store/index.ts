...
export const getShareUrl = (
  ...
  const uri = resolveUrl(`/viewer2/${worksId}${queryString ? `?${queryString}` : ''}`);
...
class ActivitiveStore {
  ...
  get isSVip() {
    return this.customerVips.some(item => item.roleAlias === 'jiantie_svip_super');
  }
  /** 待执行的分享任务（付费成功后自动执行） */
   ...
   * 跳转到编辑器
   */
  toMobileEditor = (worksId: string, uid: string, options?: NavigationOptions) => {
   ...
   * @param options 导航配置
   */
  toVideoShare = async (worksId: string, uid?: string, options?: NavigationOptions) => {
   ...
   * 供业务组件直接调用
   */
  checkSharePermission = async (worksId: string, options?: SharePermissionOptions): Promise<boolean> => {
  ...
  private setupSharePaySuccessListener = () => {
    ...
    EventEmitter.on('paySuccess', async () => {
      ...
      if (this.sharePendingTask) {
        console.log('[ActivitiveStore] 开始执行待分享任务:', this.sharePendingTask.method);
   ...
   * 用于权限拦截后，用户付费成功自动重试
   */
  private saveSharePendingTask = (method: 'shareLink' | 'shareImages' | 'shareWork', params: any): Promise<void> => {
  ...
  shareWechatVideo = (params: ShareVideoParams) => {
    const { title, content = '', thumb = '', url, scene = '0', appid = getAppId() } = params;
  ...
  shareMiniProgram = (params: ShareMiniProgramParams) => {
    const { webpageUrl, path, title, description = '', thumb = '', appid = getAppId() } = params;
   ...
   * 系统原生分享（Web API）
   */
  private shareSystemShare = async (data: { title?: string; text?: string; url?: string }) => {
