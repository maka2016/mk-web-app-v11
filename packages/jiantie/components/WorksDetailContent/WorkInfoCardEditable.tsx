...
export function WorkInfoCardEditable({
  ...
  const hasAutoGeneratedRef = useRef(false); // 标记是否已经自动生成过
  ...
  // 初始化时自动生成标题和描述（如果 is_title_desc_modified === false）
  useEffect(() => {
    const autoGenerate = async () => {
      ...
      try {
        ...
        // 检查是否需要生成封面（独立于标题描述的状态）
        if (!detail.cover || /gif/gi.test(detail.cover)) {
          console.log('[WorkInfoCard] 需要生成封面');
          needGeneration = true;
          tasks.push(generateScreenshot());
        }

        // 判断是否需要自动生成标题和描述（需要 worksData 且 is_title_desc_modified === false）
        if (worksData && !detail.is_title_desc_modified) {
          console.log('[WorkInfoCard] 需要生成标题和描述');
          needGeneration = true;
          tasks.push(generateMeta(worksData));
        }

        // 如果有需要生成的内容，显示 loading 并等待所有任务完成
        if (needGeneration && tasks.length) {
          setGeneratingContent(true);
          try {
            const results = await Promise.all(tasks);

            // 处理封面生成结果（先处理，因为可能没有标题描述生成）
            ...
            if (metaResult && metaResult.title) {
              ...
              console.log('[WorkInfoCard] 使用生成的标题:', metaResult.title);
            ...
            console.error('[WorkInfoCard] 生成内容时出错:', error);
          } finally {
            setGeneratingContent(false);
          }
        }
      } catch (error) {
        console.error('[WorkInfoCard] 自动生成失败:', error);
        setGeneratingContent(false);
      }
    };
  ...
  const generateScreenshot = async () => {
    try {
      ...
      console.log('[WorkInfoCard] 截图生成成功:', coverUrl);
      return { success: true, cover: coverUrl };
    } catch (error) {
      console.error('[WorkInfoCard] 生成截图失败:', error);
  ...
  const generateMeta = async (worksData: IWorksData) => {
    try {
      ...
      if (!workText) {
        console.log('[WorkInfoCard] 未找到文本内容，使用默认标题');
        return {
          success: false,
          title: '我的作品',
          desc: '欢迎查看',
      ...
      console.log('[WorkInfoCard] 提取的文本长度:', workText.length);
      ...
      if (metaRes && metaRes.title && metaRes.desc) {
        console.log('[WorkInfoCard] AI生成结果:', {
          title: metaRes.title,
          desc: metaRes.desc,
        });
        ...
        console.log(
          '[WorkInfoCard] 标题描述已保存，is_title_desc_modified 设置为 true'
        );
        ...
        console.warn('[WorkInfoCard] AI 返回结果格式不正确:', metaRes);
        return {
          success: false,
          title: '我的作品',
      ...
      console.error('[WorkInfoCard] 生成标题描述失败:', error);
      return {
        success: false,
        title: '我的作品',
        desc: '欢迎查看',
  ...
  const regenerateContent = async () => {
    ...
    try {
      ...
      console.error('[WorkInfoCard] 重新生成内容失败:', error);
      toast.error('生成失败，请重试');
    } finally {
      setGeneratingContent(false);
    }
