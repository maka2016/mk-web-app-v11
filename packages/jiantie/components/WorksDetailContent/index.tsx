...
function WorkDetailContentInner({
  ...
  const t = useTranslations('WorkDetailContent');
  ...
  if (!currentWork) {
    return (
      <div className='flex items-center justify-center p-8'>
        <div className='text-gray-500'>{t('作品不存在')}</div>
      </div>
    );
  }
          ...
          <div className='px-4 mb-4'>
            <div className='bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden shadow-sm'>
              ...
              <div className='bg-gray-100 border-b border-[#e2e8f0] px-4 py-2'>
                <p className='text-xs font-semibold text-[#64748b]'>{t('回执表单')}</p>
              </div>
                  ...
                  <div>
                    <div className='flex items-center gap-2'>
                      ...
                      {formSubmissionsCount && (
                        ...
                        >
                          {t('{count}个提交', { count: formSubmissionsCount })}
                        </Badge>
                      )}
                    </div>
                    <p className='text-xs text-[#90a1b9]'>
                      {rsvpConfig?.enabled ? t('正在收集信息') : t('未开启回执')}
                    </p>
                  </div>
                </div>
                          ...
                          rsvpStats.invited > 0 ? (
                            ...
                            <div className='flex items-center gap-1.5'>
                              <Badge
                                ...
                                className='bg-blue-50 text-[#2563eb] px-2 py-0.5 text-xs'
                              >
                                {t('已邀请{count}', { count: rsvpStats.invited })}
                              </Badge>
                              <Badge
                                variant='default'
                                className='bg-green-50 text-[#16a34a] px-2 py-0.5 text-xs'
                              >
                                {t('已回应{count}', { count: rsvpStats.replied })}
                              </Badge>
                            </div>
                          ) : (
                            // 未开启或没有邀请
        ...
        {/* 分享方式 */}
        {
          <div className='px-4 mb-4'>
            <div className='bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden shadow-sm'>
              {/* 标题 */}
              <div className='bg-gray-100 border-b border-[#e2e8f0] px-4 py-2'>
                <p className='text-xs font-semibold text-[#64748b]'>{t('分享方式')}</p>
              </div>
              ...
              <div className='p-2'>
                <div className='grid grid-cols-4 items-center gap-2'>
                  ...
                  {/* 更多分享方式 */}
                  {isOversea && isWebsite && !environment.isInMiniP && (
                    <VerticalActionButton
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className='w-[320px]'>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('确定要删除作品{title}吗', { title: currentWork?.title })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            >
              {worksActions.isOperating ? t('删除中') : t('删除')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
