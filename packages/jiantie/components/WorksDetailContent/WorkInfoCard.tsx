...
export function WorkInfoCard({
  ...
  const getVersionLabel = () => {
    ...
    return (
      <div
        className={cls(
          `absolute top-1.5 right-1.5 px-1.5 py-0.5 ${isPurchased ? (getAppId() === 'janur' ? 'bg-[#fdc741] text-black' : getAppId() === 'maka' ? 'bg-[#3358d4]' : 'bg-red-500') : 'bg-gray-500'} rounded text-white text-[10px] leading-tight`
        )}
      >
        ...
        >
          ...
          >
            ...
            <div className='text-xs py-1 px-2 md:text-xs absolute left-0 bottom-0 bg-black/50 text-white'>
              {specDisplayName ? (getIsOverSeas() ? tWorks(specDisplayName) : specDisplayName) : t('newVersion')}
            </div>
            {/* 已下线标签 */}
