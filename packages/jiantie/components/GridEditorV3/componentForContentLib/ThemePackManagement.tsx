...
function ThemePackManagement() {
  ...
  // 共享的主题包数据管理（用于关联主题包和主题包管理）
  // 合并为一个数据源，因为它们都访问相同的主题包资源库
  const themeResources = useMaterialResources({
    materialManager: themePackV3Manager,
    mountToLoadData: true,
    pageSize,
    // 不限制 fields，获取完整数据，因为两个功能都需要完整信息
  });
  ...
  // 关联主题包相关变量
  const hasConnectThemePack = !!gridProps.themePackV3RefId?.documentId;
  ...
  const handleConnectThemePack = async (materialItem: any) => {
    ...
    if (themePackWorksId) {
      const themeWorksData = await trpc.works.getWorksData.query({
        id: themePackWorksId,
      });
      const gridProps = themeWorksData.work_data.gridProps;
      const worksDetail =
        themeWorksData.detail as unknown as SerializedWorksEntity;
      worksStore.api.updateWorksDetail({
        envelope_config: worksDetail.envelope_config as any,
        envelope_enabled: worksDetail.envelope_enabled || false,
      });
      worksStore.setGridProps({
        themePackV3RefId: {
  ...
  const handleImportThemePack = async (materialItem: any) => {
    ...
    if (content) {
      ...
      if (worksIdFromThemePack) {
        const worksDataFromTheme = await trpc.works.getWorksData.query({
          id: worksIdFromThemePack,
        });
        const worksDetail =
          worksDataFromTheme.detail as unknown as SerializedWorksEntity;
        const gridPropsFromTheme = worksDataFromTheme.work_data.gridProps;
        worksStore.api.updateWorksDetail({
          envelope_config: worksDetail.envelope_config,
          envelope_enabled: worksDetail.envelope_enabled || false,
        });
        if (gridPropsFromTheme) {
          worksStore.setGridProps({
            materialResourcesGroup: toJS(
              gridPropsFromTheme.materialResourcesGroup
            ) as Partial<MaterialResourcesGroup>,
            themeConfig2: toJS(gridPropsFromTheme.themeConfig2),
            _updateVersion: (gridProps?._updateVersion || 0) + 1,
          });
        }
      }
    ...
    <div className='flex flex-col gap-4 p-2'>
      ...
      <div className='flex flex-col gap-2 border rounded-md p-2'>
        ...
        <div className='flex gap-2'>
          ...
          >
            {hasConnectThemePack ? '已关联主题包，点击切换' : '关联主题包'}
          </Button>
          {hasConnectThemePack && (
            <>
              <Button
              ...
              <Button
                ...
                onClick={() => {
                  window.open(
                    `/desktop/editor-designer?works_id=${gridProps.themePackV3RefId?.worksId}&designer_tool=1&no_save=1`
                  );
                }}
              >
                查看
