    ...
    function KeyCheck(event: KeyboardEvent) {
      ...
      const toDesignerMode = () => {
        ...
        if (isCommand && k === 'i') {
          ...
          if (/^T_/.test(id)) {
            // 编辑模版
            const newUrl = `/desktop/editor-designer?works_id=${id}&designer_tool=2&uid=${getUid()}&is_template=true`;
        ...
        if (isCommand && event.key === 'k') {
          ...
          const isViewer = window.location.pathname.includes('/viewer2/');
          let worksId = '';
      ...
      const copyPasteMonitor = () => {
        if (
          isTyping ||
          (!editingElemId &&
            (hideOperator || !activeRowDepth || activeRowDepth.length === 0))
        ) {
          return;
        }
        if (!isCommand) return;
        ...
        switch (k) {
          ...
          case 'ArrowLeft':
            ...
            if (!editingElemId) {
              moveRowV2('up');
              const lastIndex =
                activeRowDepth?.[activeRowDepth.length - 1] || 0;
              setWidgetStateV2({
      ...
      const animationPlayer = () => {
        ...
        if (k === '-') {
          setWidgetStateV2({
            playAnimationInEditor: !widgetStateV2.playAnimationInEditor,
          });
        }
      };
      animationPlayer();
