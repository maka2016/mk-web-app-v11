...
import { useAppAnimate2 } from '../components/AnimationTool/useAnimate2';
...
const WidgetItemRendererV2InternalComponent = (
  ...
  // 检查元素是否配置了点击动作（用于 viewer 模式下启用交互）
  const hasClickAction = layer.action?.enable && !!layer.action?.type;

  // 【关键修复】检查是否有进场动画，如果有则设置初始隐藏状态
  // 这确保元素在首次渲染时就是隐藏的，避免"闪现"问题
  const hasEntranceAnimation = animateQueue2?.entrance && animateQueue2.entrance.length > 0;
  const initialHiddenStyle = hasEntranceAnimation && autoPlayAnimation && inViewer
  ...
  const wrapperStyle = blockStyleFilter({
    ...
    alignItems: 'center',
    display: 'grid',
    pointerEvents: editable || isActive || (inViewer && hasClickAction) ? 'auto' : 'none',
    cursor: inViewer && hasClickAction ? 'pointer' : undefined,
    zIndex: defaultZIndex,
    ...animationStyle,
    ...initialHiddenStyle, // 应用初始隐藏样式（如果有进场动画）
  }) as React.CSSProperties;
  ...
  return (
    <LayoutWrapper
      ...
      onClick={e => {
        // viewer 模式下执行点击动作
        if (inViewer && layer.action?.enable) {
          e.stopPropagation();
          const { type, actionAttrs } = layer.action;
          if (type === 'link' && actionAttrs?.link) {
            window.open(
              actionAttrs.link as string,
              actionAttrs.openInNewTab !== false ? '_blank' : '_self'
            );
          } else if (type === 'goToPage' && typeof actionAttrs?.pageIndex === 'number') {
            window.dispatchEvent(
              new CustomEvent('go_to_page', {
                detail: { pageIndex: actionAttrs.pageIndex },
              })
            );
          } else if (type === 'playAnimation') {
            // 设置全局标志并触发已有的动画就绪事件，useAnimate2 会自动响应并播放
...
const COMPARE_KEYS = {
  ...
  layerLinkComplex: ['animateQueue', 'animateQueue2', 'action'] as const,
};
...
const WidgetItemRendererV2Internal = memo(
