...
import { GridRow, GridState, ThemeConfigV2 } from '../../types';
...
export interface AddConfigContext {
  /** 添加组件到画布 */
  addComponentV2: (params: { layer: AddComponentParams; toIndex?: number }) => string | undefined;
  /** 添加行到根节点 */
...
export const createAddHandlers = (context: AddConfigContext) => {
  const { addComponentV2, widgetStateV2, addRowToRowChildV2, setWidgetStateV2 } = context;

  return {
    // 基础元素处理器
    handleBasicElement: (addItem: IAddItem) => {
      if (!widgetStateV2.activeRowDepth || widgetStateV2.activeRowDepth.length <= 1) {
 ...
 * 获取互动组件列表
 */
export const getInteractiveComponents = (handleInteractiveComponent: (component: any) => Promise<void>): IAddItem[] => {
...
export const getAddContainerThemeConfig = (themeConfig: ThemeConfigV2): IAddItem[] => {
  return [
...
export const getAddPictureItemsByThemeConfig = (themeConfig: ThemeConfigV2): IAddItem[] => {
  return [
    {
...
export const getAddTextsByThemeConfig = (themeConfig: ThemeConfigV2): IAddItem[] => {
  return [
    {
