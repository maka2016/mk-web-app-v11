...
import { ComponentContent, ComponentData, ComponentGroupData, ComponentGroupDataItem } from '../types';
...
const createGroupDataManager = (config: GroupDataManagerConfig) => {
  ...
  const addToGroup = (groupId: string, nextCompData: ComponentData, groupName: string) => {
    const groupData = getGroupData();
    if (!groupData) {
      throw new Error(`${dataName}分组数据不存在`);
    }
    const nextGroupData = deepClone(groupData);
    const currGroupIdx = nextGroupData.findIndex((c: ComponentGroupDataItem) => c.groupId === groupId);

    if (currGroupIdx !== -1) {
      if (nextGroupData[currGroupIdx].datas.find((d: ComponentData) => d.compId === nextCompData.compId)) {
  ...
  const updateData = (groupId: string, compData: ComponentData) => {
    ...
    const nextGroupData = deepClone(groupData);
    const currGroupIndex = nextGroupData.findIndex((c: ComponentGroupDataItem) => c.groupId === groupId);

    if (currGroupIndex !== -1) {
      const targetIdx = nextGroupData[currGroupIndex].datas.findIndex(
        (d: ComponentData) => d.compId === compData.data.rows[0].sourceComponentId
      );
      if (targetIdx !== -1) {
        const compName = nextGroupData[currGroupIndex].datas[targetIdx].compName;
        nextGroupData[currGroupIndex].datas[targetIdx] = compData;
        nextGroupData[currGroupIndex].datas[targetIdx].compName = compName;
        updateGroupData(nextGroupData);
      } else {
        console.log(`找不到要更新的${dataName}组件，请检查代码`, groupId, compData);
  ...
  const deleteGroup = (groupId: string) => {
    ...
    const nextGroupData = deepClone(groupData);
    const groupIdx = nextGroupData.findIndex((g: ComponentGroupDataItem) => g.groupId === groupId);
  ...
  const getItemById = (groupId: string, compId: string) => {
    ...
    const groupIdx = nextGroupData.findIndex((g: ComponentGroupDataItem) => g.groupId === groupId);
    if (groupIdx !== -1) {
      return nextGroupData[groupIdx].datas.find((c: ComponentData) => c.compId === compId);
    }
    return undefined;
  };
  ...
  const moveItemToGroup = (componentId: string, targetGroupId: string) => {
    ...
    for (let i = 0; i < newData.length; i++) {
      const idx = newData[i].datas.findIndex((c: ComponentData) => c.compId === componentId);
      if (idx !== -1) {
        sourceGroupIdx = i;
        componentIndex = idx;
        break;
    ...
    // 找到目标组
    const targetGroupIdx = newData.findIndex((g: ComponentGroupDataItem) => g.groupId === targetGroupId);
    if (targetGroupIdx === -1) {
    ...
    if (newData[sourceGroupIdx].datas.length === 0) {
      finalData = newData.filter((g: ComponentGroupDataItem) => g.groupId !== sourceGroupId);
    }
...
export class ThemePackV3Operator {
  ...
  /** 获取 content 中的 style 等扩展字段，用于 updateItem 时合并，避免覆盖 */
  private getContentExtras?: () => Partial<Pick<ComponentContent, 'materialResourcesGroup' | 'themeConfig2'>>;
  ...
  private _setComponentGroupData = (data: ComponentGroupData) => {
    ...
    if (this.canUpdate && this.themePackDocumentId) {
      themePackV3Manager.updateItem(this.themePackDocumentId, {
        content: {
          ...(this.themePackWorksId != null && { worksId: this.themePackWorksId }),
          componentsGrids: data,
          componentsBlocks: this.blockGroupData || [],
          ...this.getContentExtras?.(),
        },
      });
    }
  };
   ...
   * @param themePackV3RefId 主题包引用 ID（documentId 必填，worksId 可选仅作展示）
   * @param canUpdate 是否可以更新
   * @param componentContent 主题包数据
   * @param getContentExtras 主题包模式下由调用方传入，用于 updateItem 时合并 materialResourcesGroup/themeConfig2
   */
  init = (
    themePackV3RefId: string,
    canUpdate: boolean,
    componentContent?: ComponentContent,
    getContentExtras?: () => Partial<Pick<ComponentContent, 'materialResourcesGroup' | 'themeConfig2'>>
  ) => {
    this.canUpdate = canUpdate;
    this.themePackDocumentId = themePackV3RefId;
    this.themePackWorksId = componentContent?.worksId;
    this.getContentExtras = getContentExtras;

    if (!this.themePackDocumentId) {
      throw new Error('主题包 documentId 不能为空');
    }

    runInAction(() => {
      this.componentGroupData = componentContent?.componentsGrids || [];
      this.blockGroupData = componentContent?.componentsBlocks || [];
      this.loading = false;
    });
  };
   ...
   * 添加组件到分组
   */
  addComponentToGroup = (groupId: string, nextCompData: ComponentData, groupName: string) => {
   ...
   * 添加区块到分组
   */
  addBlockToGroup = (groupId: string, nextCompData: ComponentData, groupName: string) => {
  ...
  /**
   * 触发式同步：将当前 content（含 componentsGrids、componentsBlocks、materialResourcesGroup、themeConfig2）写回素材项
   * 由 UI 在对应页面上编辑后主动调用，不做自动监听
   */
  syncContentToMaterialItem = () => {
    if (!this.canUpdate || !this.themePackDocumentId) return;
    const extras = this.getContentExtras?.() ?? {};
    const fullContent = {
      ...(this.themePackWorksId != null && { worksId: this.themePackWorksId }),
      componentsGrids: this.componentGroupData || [],
      componentsBlocks: this.blockGroupData || [],
      materialResourcesGroup: extras.materialResourcesGroup != null ? toJS(extras.materialResourcesGroup) : undefined,
      themeConfig2: extras.themeConfig2 != null ? toJS(extras.themeConfig2) : undefined,
    };
    themePackV3Manager.updateItem(this.themePackDocumentId, { content: fullContent });
  };
}
