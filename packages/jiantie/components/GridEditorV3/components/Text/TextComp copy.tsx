...
import { isIOS, SerializedWorksEntity } from '@/utils';
...
import { computeCroppedFontId } from '../../utils/font-utils';
import { loadFontAction } from '../../utils/load-font';
  ...
  useEffect(() => {
    ...
    const loadCropFont = async () => {
      if (loadFullFont) {
        ...
        // viewer 预览模式：用 computeCroppedFontId 本地计算裁剪字体 URL，省去一次 batch 接口请求
        const { url: cropUrl } = computeCroppedFontId(text, fontFamily);
        await loadFont({
          fontFamily,
          fontUrl: cropUrl,
