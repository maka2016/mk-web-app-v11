...
const Tools = (props: Props) => {
  ...
  const [CompData, setCompData] = useState<
    Record<string, LayerElemItem | undefined>
  >({});
  ...
  useEffect(() => {
    const layers: LayerElemItem[] = getAllLayers(props.worksData);
    ...
    for (const layer of layers) {
      if (layer.elementRef in compMap && compMap[layer.elementRef] === null) {
        compMap[layer.elementRef] = layer;
      }
    }

    setCompData(compMap);
    if (compMap.MkHuiZhi) {
      if (getCookie(onceSubmitCookiesKey(compMap.MkHuiZhi.attrs.formRefId))) {
  ...
  const onSubmit = async () => {
    ...
    if (CompData.MkGift?.attrs.show !== false && giftItem) {
      if (!giftItem) {
        return;
      }
      request.push(
        formReceiverServiceApi.formSubmit(CompData.MkGift?.attrs.formRefId, {
          formData: {
    ...
    if (CompData.MkBulletScreen_v2?.attrs.show !== false) {
      request.push(
        formReceiverServiceApi.formSubmit(
          CompData.MkBulletScreen_v2?.attrs.formRefId,
          {
            formData: {
              content: comment,
              nickname: formData.name,
    ...
    request.push(
      formReceiverServiceApi.formSubmit(
        CompData.MkHuiZhi?.attrs.formRefId,
        submitData
      )
    );
    ...
    setCookieExpire(
      onceSubmitCookiesKey(CompData.MkHuiZhi?.attrs.formRefId),
      'true'
    );
    setOverOnceSubmit(true);
  ...
  if (!CompData.MkHuiZhi || CompData.MkHuiZhi?.attrs.show === false) {
    return <></>;
  }
        ...
        {overOnceSubmit ? (
          <div>
            <div className={styles.success_content}>
              <div className={styles.submit_success}>
                ...
                />
                <span>
                  {CompData.MkHuiZhi?.attrs.feedback ||
          ...
          <>
            ...
            <div className={styles.content}>
              ...
              {CompData.MkBulletScreen_v2?.attrs.show !== false && (
                <>
              ...
              {CompData.MkGift?.attrs.show !== false && (
                <>
