    ...
    <>
      <TempLinkBarContainer>
        <div className='tempLinkText'>
          {t('tempLinkExpire', { date: formatExpireDate(expireDate) })}
          {!isGlobal && t('tempLinkOr')}
        </div>
        {!isGlobal && (
          <>
            <BehaviorBox
            ...
            <span>{t('tempLinkUpgradeSuffix')}</span>
          </>
        )}
        <span className='flex-1'></span>
        <X size={12} onClick={() => setHideBar(true)} />
      </TempLinkBarContainer>
      {!isGlobal && <ResponsiveDialog
        isDialog
