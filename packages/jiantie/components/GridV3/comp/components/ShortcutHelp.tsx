...
const ShortcutHelp: React.FC<ShortcutHelpProps> = ({
  className,
  targetContainer = '#designer_scroll_container',
}) => {
  const shortcuts = [
    {
      category: '编辑操作',
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant='outline'
          size='xs'
          className={`fixed bottom-[8px] right-[8px] z-[9999] ${className || ''}`}
          title='快捷键说明'
        >
          <HelpCircle size={12} />
          <span>快捷键</span>
  ...
  // 使用React Portal将组件渲染到目标容器
  return ReactDOM.createPortal(shortcutContent, document.body);
};
