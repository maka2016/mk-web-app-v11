...
const EditFormForCopyableCell = () => {
  ...
  const renderTextForm = (layer: LayerElemItem, cell: GridRow) => {
    const { attrs } = layer;
  ...
  const renderPictureForm = (layer: LayerElemItem, cell: GridRow) => {
    const { attrs } = layer;
  ...
  const renderElementForm = (layer: LayerElemItem, cell: GridRow) => {
    if (layer.elementRef === 'Text') {
  ...
  const handleDuplicateCell = (cells: GridRow[]) => {
    handleChangeRepeatList((currRow.children?.length || 0) + 1);
  ...
  const handleDeleteCell = (cells: GridRow[]) => {
    // deleteCellBatch(
  ...
  const renderCellForm = (cell: GridRow) => {
    return (
      <div
        style={{
          display: 'flex',
    ...
    <EditFormContainer>
      ...
      >
        {cells.map((cellItem, index) => {
          ...
          // const cellIndex = cellItems.findIndex(
          //   (c) => c.id === cellItems[0].id
          // );
          ...
          return (
            <FormSection
              ...
              key={`cell-${cellItem.id}_${index}`}
              isActive={isActive}
              data-cell-id={cellItem.id}
              onClick={e => {
                e.stopPropagation();
                handleCellActivate(cellItem.id);
              }}
            >
              <FormSectionHeader>
