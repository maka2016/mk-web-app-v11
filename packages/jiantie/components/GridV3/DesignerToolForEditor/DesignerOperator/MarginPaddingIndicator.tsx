...
import { stringValueTo4Chunk } from '../ElementAttrsEditor/utils';
...
const MarginPaddingIndicator: React.FC<MarginPaddingIndicatorProps> = ({
  targetElement,
  widgetState,
}) => {
  ...
  const updateStyleValues = () => {
    ...
    setIsAbsolutePositioned(isAbsolute);
    ...
    setAbsoluteValues(absoluteChunks);
  ...
  }, [targetElement, editingElemId, activeCellId, activeRowId]); // 添加 widgetState 相关依赖
  ...
  }, [editingElemId, activeCellId, activeRowId]);
  ...
  if (!targetElement || (!editingElemId && !activeCellId && !activeRowId)) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      {/* 绝对定位指示器 - 只在元素为绝对定位时显示 */}
      {/* {isAbsolutePositioned && (
        <AbsolutePositionIndicator
          id="AbsolutePositionIndicator"
          top={absoluteValues[0] + Math.abs(marginValues[0])}
          right={0}
          bottom={0}
          left={absoluteValues[3] + marginValues[3]}
          isVisible={true}
        >
          <ArrowIndicator
            direction="top"
            value={absoluteValues[0]}
            isVisible={absoluteValues[0] !== 0}
          />
          <ArrowIndicator
            direction="left"
            value={absoluteValues[3]}
            isVisible={absoluteValues[3] !== 0}
          />

          {absoluteValues[0] !== 0 && (
            <ValueLabel position="top" type="absolute">
              {absoluteValues[0]}
            </ValueLabel>
          )}
          {absoluteValues[1] !== 0 && (
            <ValueLabel position="right" type="absolute">
              {absoluteValues[1]}
            </ValueLabel>
          )}
          {absoluteValues[2] !== 0 && (
            <ValueLabel position="bottom" type="absolute">
              {absoluteValues[2]}
            </ValueLabel>
          )}
          {absoluteValues[3] !== 0 && (
            <ValueLabel position="left" type="absolute">
              {absoluteValues[3]}
            </ValueLabel>
          )}
        </AbsolutePositionIndicator>
...
export default MarginPaddingIndicator;
