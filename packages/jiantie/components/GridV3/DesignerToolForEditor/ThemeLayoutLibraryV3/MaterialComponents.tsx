...
import LongPageRowEditorV2 from '../../comp/components/RowRendererV2/LongPageRowEditorV2';
...
function GroupHeader({
  ...
  const { manager, handleGroupRename, handleGroupDelete } =
    useMaterialComponentsContext();
  ...
  const newData: IWorksData = {
    ...
    canvasData: {
      ...
      content: {
        pages: [
          {
            layers: component.data.elemComps,
            id: random(),
            opacity: 1,
    ...
    positionLink: component.data.positionLink,
  };
  return newData;
};
...
function ComponentNameEditor({ component }: { component: ComponentData }) {
  const { manager, handleComponentRename } = useMaterialComponentsContext();
  const [isEditing, setIsEditing] = useState(false);
  ...
  return isEditing ? (
    ...
    <div
      className={`page_name h-5 text-xs ${manager ? 'cursor-text hover:bg-black/70' : ''}`}
      onClick={handleClick}
    >
      {component.compName || '未命名'}
    </div>
...
function ComponentItemWrapper({
  ...
  return (
    <div
      ...
      className={clas('row_wrapper relative', {
...
function ComponentGroupContent({
  ...
  const renderComponentList = () => {
    return filteredComponents.map(component => {
      ...
      const gridsData: GridRow[] = [
        {
          id: `temp_id_${component.compId}`,
          cells: [],
          children: [
            {
              id: `temp_cell_id_${component.compId}`,
              children: component.data.rows,
              cells: [],
              style: getStyleByTag2('block'),
            },
          ],
          style: {
            ...(getStyleByTag2('page') || {}),
...
interface MaterialComponentsProps {
  ...
  /** 一行的列数，默认 2 列 */
  gridColumns?: number;
  /** 视图模式：vertical 纵向瀑布流（已废弃 horizontal 横向滚动模式） */
  viewMode?: ViewMode;
...
export default function MaterialComponents({
  ...
  const rawGroupData =
    dataType === 'components'
      ? themePackV3Data.componentGroupData
  ...
  const setGroupData =
    dataType === 'components'
      ...
      : themePackV3Data.setBlockGroupData;

  const deleteGroupData =
    dataType === 'components'
      ? themePackV3Data.deleteComponentGroupData
      : themePackV3Data.deleteBlockGroupData;

  const moveComponentToGroup =
    dataType === 'components'
      ? themePackV3Data.moveComponentToGroup
      : themePackV3Data.moveBlockToGroup;

  const { worksData, themePackGridProps } = themePackV3Data;
  ...
  if (!worksData || !themePackGridProps) {
    return (
    ...
    <MaterialComponentsContext.Provider value={contextValue}>
      ...
      >
        ...
        <div className='flex-1 flex overflow-hidden'>
          ...
          {showGroupNavigation && navigationGroups.length > 0 && (
            <GroupNavigation
