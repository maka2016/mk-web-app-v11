...
import { Plus, Minus, Check, ChevronDown, Settings } from 'lucide-react';
...
import { extractDropShadow, extractOtherFilters } from './utils';
...
export default function AppearanceSettings({
  ...
  const [borderImageDialogOpen1, setBorderImageDialogOpen1] = useState(false);
  const [borderImageDialogOpen2, setBorderImageDialogOpen2] = useState(
    !!(style as any).borderImage2
  );
    ...
    <>
      <div className='flex flex-col gap-2 bg-custom-gray rounded-sm py-1'>
        <div className='px-2 flex items-center justify-between'>
          <Label className='text-xs'>9宫格背景</Label>
        ...
        {borderImageDialogOpen2 && (
          <div>
            <BorderImageSliceEditor
              targetDOM={targetDOM}
              value={(style as any).borderImage2 || {}}
              onChange={value => {
          ...
          <>
            <div className='flex items-center px-2'>
              <ColorPickerPopover
                ...
                onChange={value => {
                  onChange({
                    borderColor: value ? value.hex || value.value : undefined,
                  });
                }}
              />
              <div
