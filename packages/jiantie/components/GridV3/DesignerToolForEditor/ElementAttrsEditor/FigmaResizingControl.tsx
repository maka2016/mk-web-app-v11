...
interface FigmaResizingControlProps {
...
const heightModeOptions = [
  { value: 'auto', label: '适配' },
  { value: 'min-height', label: '最小' },
  { value: 'normal-height', label: '常规' },
];

export default function FigmaResizingControl({
  dimension,
  value,
  onChange,
}: FigmaResizingControlProps) {
  const currentValue = (() => {
    if (dimension === 'width') {
      return value.width;
    } else {
  ...
  const handleFixedValueChange = (inputValue: string) => {
    ...
    if (!isNaN(numValue)) {
      if (dimension === 'width' && mode === 'fixed-percent') {
        const percentValue = `${numValue}%`;
        onChange({
          [dimension]: percentValue,
          [minDimension]: percentValue,
        });
      } else {
        onChange({
          [dimension]: numValue,
          [minDimension]: numValue,
        });
      }
    }
  };
  ...
  const getPlaceholder = () => {
    if (dimension === 'width') {
      return mode === 'fixed-percent' ? '50' : '100';
    } else {
      return '100';
    }
  };
