  ...
  try {
    ...
    if (!verifyResult.success || !verifyResult.data) {
      ...
      await prisma.paymentCallbackLog.update({
        ...
        data: {
          status: 'failed',
          error_message: verifyResult.error || '苹果服务器验证失败',
    ...
    if (!transaction) {
      ...
      await prisma.paymentCallbackLog.update({
        ...
        data: {
          status: 'failed',
          error_message: '无法从 receipt 中提取有效的交易信息',
          process_time: new Date(),
        },
      });
    ...
    if (!product) {
      ...
      await prisma.paymentCallbackLog.update({
        ...
        data: {
          status: 'failed',
          error_message: `未找到对应的商品: ${transaction.product_id}`,
          process_time: new Date(),
          parsed_data: {
            ...(callbackLog.parsed_data as object),
            transaction_id: transaction.transaction_id,
    ...
    try {
      ...
      console.error('[Apple IAP] 更新回调日志失败:', logError);
    }
