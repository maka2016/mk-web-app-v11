...
const SearchFacets = (props: Props) => {
  ...
  return (
    <div className='px-4 pb-2 relative bg-white'>
      <div className='flex items-center justify-between py-1'>
        ...
        <div
          className={cls([
            'flex items-center text-[#01070d33] text-sm font-normal leading-[22px]',
            active === 'orderby' && 'text-[var(--theme-color)]',
          ])}
          onClick={() => {
            if (active === 'orderby') {
              setActive('');
        ...
        >
          ...
          <Icon
            name={active === 'orderby' ? 'arrow-up' : 'arrow-down'}
            size={12}
          />
        </div>
      </div>
      {active && (
        <div className='absolute top-[99%] left-0 right-0 pt-1 px-4 bg-white z-[9] overflow-hidden overscroll-none'>
          {active === 'orderby' && (
            <div>
              {AllOrder.map(item => (
                <div
                  key={item.id}
                  className={cls([
                    'h-[46px] w-full text-center text-[#333] text-sm font-normal leading-[46px]',
                    (searchParams.orderBy || '') === item.val &&
                      'text-[var(--theme-color)] font-semibold',
                  ])}
                  onClick={() => onChangeOrderType(item.val)}
                >
                  {item.name}
                </div>
              ))}
          ...
          {active === 'spec' && (
            <div className='flex flex-wrap max-h-[40vh] overflow-y-auto overscroll-contain'>
              <BehaviorBox
                behavior={{
          ...
          {active === 'more' && (
            <div>
              <p className='text-black text-sm font-semibold leading-[22px] mb-3'>
                颜色
              </p>
              <div
                className='flex flex-wrap max-h-[40vh] overflow-y-auto overscroll-contain'
