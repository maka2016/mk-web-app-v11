...
const WaterfallFloor2 = (props: Props) => {
  const { channelId, fixed, appid } = props;
  ...
  const activeFloor = tags.find(item => item.documentId === tagId);
  ...
  const getTags = async () => {
    const query = qs.stringify(
      {
        populate: {
          template_tag_2s: {
            populate: '*',
            fields: ['id', 'documentId', 'name', 'desc', 'filter', 'sort'],
          },
        },
        filters: {
          documentId: {
    ...
    const promptGroupRes = (
      await requestCMS.get(`${getCmsApiHost()}/api/template-channels?${query}`)
    ).data.data;

    if (promptGroupRes.length > 0) {
      const data = promptGroupRes[0].template_tag_2s;
      // Sort tags by sort field in ascending order
  ...
  const getFloorTemplates = async () => {
    ...
    let xsTemplates = [];
    if (page === 1) {
      xsTemplates = await getNewProtectTemplates(activeFloor.name, 2);
    ...
    console.log(
      'activeFloor?.filter?.default?.$and',
      activeFloor?.filter?.default?.$and
    );
  ...
  const renderTags = () => {
    if (tags.length <= 1) return <div className='h-3'></div>;
    return (
      <div
        className={cls([
          styles.floorTags,
  ...
  return (
    <div className='flex flex-col flex-1  bg-white relative rounded-t-xl'>
      ...
      {!!activeFloor?.filter?.facets?.length && (
        <BehaviorBox
          behavior={{
      ...
      >
        <div className={styles.filterPanel}>
          ...
          />
          {activeFloor?.filter?.facets?.map(item => {
            return (
