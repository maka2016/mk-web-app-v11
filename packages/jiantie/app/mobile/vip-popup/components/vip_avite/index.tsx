...
const AviteVip = (props: Props) => {
  ...
  const [iapPrices, setIapPrices] = useState<Record<string, IAPProductPrice>>({});
    ...
    products.forEach((item: VipPackage) => {
      ...
      item.productSkus.forEach((sku: any) => {
        ...
        const pack: WorkPricePackage = {
          ...
          googleIapProductId: sku.googleIapProductId || '',
  ...
  // 价格包加载完成后，在 App 内通过 bridge 获取 IAP 本地化价格
  useEffect(() => {
    if (!vipPackage || !APPBridge.judgeIsInApp()) return;
    const allPackages = vipPackage[activeTab] || [];
    const productIds: string[] = [];
    allPackages.forEach(pkg => {
      if (isIOS() && pkg.iapProductId) {
        productIds.push(pkg.iapProductId);
      } else if (isAndroid() && pkg.googleIapProductId) {
        productIds.push(pkg.googleIapProductId);
      }
    });
    if (productIds.length > 0) {
      fetchIAPPrices(productIds);
    }

    // AppsFlyer 打点：价格包加载完成，上报 af_initiated_checkout
    const firstPkg = allPackages[0];
    if (firstPkg) {
      APPBridge.appCall(
        {
          type: 'MKAppsFlyerLog' as any,
          params: {
            eventName: 'af_initiated_checkout',
            eventValues: {
              af_currency: firstPkg.currency || 'USD',
              af_content_type: 'vip',
            },
          },
          jsCbFnName: 'onAppsFlyerLogResult',
        },
        (cbParams: { success: boolean; msg?: string }) => {
          if (!cbParams?.success) {
            console.warn('AppsFlyer log failed:', cbParams?.msg);
          }
        },
        5000
      );
  ...
  // 通过 app bridge 获取 IAP 本地化价格
  const fetchIAPPrices = (productIds: string[]) => {
    if (!productIds.length || !APPBridge.judgeIsInApp()) return;
    ...
    APPBridge.appCall(
      {
        ...
        jsCbFnName: 'RNGETIAPPRICESCb',
      },
      (cbParams: { success: boolean; msg: string; data: Record<string, IAPProductPrice> }) => {
        if (cbParams?.success && cbParams.data) {
          setIapPrices(cbParams.data);
        }
      },
      10000
    );
      ...
      APPBridge.appCall(
        {
          type: 'RNIAPPAY' as any,
          params: {
            productId: cuPackage.googleIapProductId,
            trackData: {
              workId: vipTrackData.works_id,
              ...vipTrackData,
            },
  ...
  const userAgreement = () => {
    const url =
      'https://res-ai.maka.im/janur/app/janur-user-agreement.html';
    if (APPBridge.judgeIsInApp()) {
      // APPBridge.navToPage({
      //   url,
      //   type: 'URL',
      ...
      APPBridge.appCall(
        ...
        data => {
          console.log('MKOpenBrowser', data);
        },
        5000
      );
  ...
  // 获取 IAP 本地化展示价格，如果 IAP 价格不可用则回退到服务端价格
  const getDisplayPrice = (pkg: WorkPricePackage | undefined) => {
    if (!pkg) return '';
    const productId = isIOS() ? pkg.iapProductId : pkg.googleIapProductId;
    const iapPrice = productId ? iapPrices[productId] : undefined;
    if (iapPrice?.displayPrice) {
      return iapPrice.displayPrice;
    }
    const symbol = pkg.currency === 'CNY' ? '¥' : '$';
    return `${symbol}${pkg.price / 100}`;
  };

  const getOriginalDisplayPrice = (pkg: WorkPricePackage | undefined) => {
    if (!pkg || !pkg.originalPrice || pkg.originalPrice === pkg.price) return '';
    const productId = isIOS() ? pkg.iapProductId : pkg.googleIapProductId;
    const iapPrice = productId ? iapPrices[productId] : undefined;
    if (iapPrice) {
      // IAP 价格可用时，使用 IAP 货币符号 + 原价换算
      const symbol = iapPrice.currencySymbol || (pkg.currency === 'CNY' ? '¥' : '$');
      return `${symbol}${pkg.originalPrice / 100}`;
    }
    const symbol = pkg.currency === 'CNY' ? '¥' : '$';
    return `${symbol}${pkg.originalPrice / 100}`;
  };
                        ...
                        annualPackage.originalPrice !== annualPackage.price && (
                          <div className={styles.annualPriceInline}>
                            <div className={styles.originalPrice}>
                              {getOriginalDisplayPrice(annualPackage)}
                            </div>
                            <div className={styles.currentPrice}>
                              {getDisplayPrice(annualPackage)}
                            </div>
                          </div>
                        )}
                    </div>
      ...
      <div className={styles.footer}>
        <Button className={styles.btnPay} size='lg' onClick={() => toPay()}>
          {paymentType === 'annual' && annualPackage
            ? t('payNowAnnual', { price: getDisplayPrice(annualPackage) })
            : singlePackage
              ? t('payNowSingle', { price: getDisplayPrice(singlePackage) })
              : t('upgradeNow')}
        </Button>
