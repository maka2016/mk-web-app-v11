...
import { EventEmitter, getUrlWithParam } from '@/utils';
...
function CreateBtn({
  ...
  const toEditor = (works_id: string) => {
    ...
    router.replace(
      getUrlWithParam(
        `/mobile/editor?works_id=${works_id}&uid=${uid}&is_full_screen=1&popEnable=0&appid=${appid}`,
        'clickid'
      )
    );
    toast.dismiss();
  ...
  const createWorks = async () => {
    ...
    try {
      ...
      const worksRes = await trpc.works.create.mutate({
        ...
        cover: templateDetail.coverV3?.url || '',
        appid: appid,
      ...
      >
        <div className='p-4'>
          <div className='text-base font-semibold text-black text-center mt-4 mb-1'>
            {t('您已有count个作品使用过此模板', { count: templateWorks.length })}
          </div>
