...
export default function WorkDetailPage() {
  ...
  // 生成邀请函海报（图片/视频规格）
  const generateInvitationPoster = async (): Promise<string[] | undefined> => {
    if (!workId) {
      toast.error('作品信息缺失');
      return;
    }
    ...
    try {
      ...
      toast.loading('生成邀请函中...', { id: 'generating' });

      // 获取作品数据和画布信息
      const res = await getWorkData2(workId);
      const detail = res?.detail;
      const worksData = res?.work_data;

      if (!detail || !worksData) {
        toast.error('获取作品信息失败', { id: 'generating' });
        return;
      }

      const canvaInfo2 = getCanvaInfo2(detail as any, worksData);
      if (!canvaInfo2) {
        toast.error('画布信息获取失败', { id: 'generating' });
        return;
      }
      ...
      toast.success('生成成功', { id: 'generating' });
      return urls;
    } catch (error: any) {
      console.error('生成海报失败:', error);
      toast.error('生成失败，请重试', { id: 'generating' });
      return;
    } finally {
