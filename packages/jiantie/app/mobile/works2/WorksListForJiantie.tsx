...
import { useWorksActions, WorksActionsProvider } from '@/components/WorksDetailContent/WorksActionsContext';
...
import { ArrowDown, ChevronLeft, ChevronRight, Loader2, Search, X } from 'lucide-react';
...
function WorksListForJiantieInner() {
  ...
  const [purchasedWorksMap, setPurchasedWorksMap] = useState<Map<string, boolean>>(new Map());
  ...
  const getPurchasedWorks = async (worksIds: string[]) => {
    ...
    if (!useV11API) {
      const res = await request.get(`${API('apiv10')}/user-resources?type=purchased&resourceIds=${worksIds.join(',')}`);
  ...
  const loadWorksSpecs = async (silent = false) => {
    ...
    try {
      ...
      const { specs, totalWorksCount } = await trpc.worksSpec.findManyWithStats.query({
  ...
  // 加载作品列表
  const loadWorks = async (pageNum = page, keyword = searchKeyword, silent = false) => {
    const currentUid = getUid();
    if (!currentUid) {
      return;
    }
    ...
    (e: TouchEvent) => {
      ...
      if (scrollContainer.scrollTop === 0 && touchStartY.current > 0) {
        ...
        if (distance > 0) {
          ...
          const dampedDistance = Math.min(distance * dampingFactor, maxDistance);
  ...
  // 获取购买状态标签
  const getPurchaseStatus = (workId: string): 'purchased' | 'not-purchased' | null => {
    // 如果是会员，不显示购买状态
    if (store.isVip) {
      return null;
    }
      ...
      >
        ...
        <div className='relative px-4 border-b border-gray-200 bg-white'>
          {!showSearch ? (
            ...
            <div className='flex items-center gap-2 py-3 border-b-2 border-transparent'>
              ...
              <button onClick={handleSearch} className='py-2 text-md text-blue-600 whitespace-nowrap font-medium'>
                {t('搜索')}
              </button>
              <button onClick={toggleSearch} className='py-2 text-md text-gray-600 whitespace-nowrap'>
                {t('取消')}
              </button>
            </div>
          )}
          ...
          {(pullDistance > 0 || refreshing) && (
            <div className='flex items-center justify-center h-full text-sm'>
              {refreshing ? (
                ...
                >
                  <ArrowDown className={`w-5 h-5 transition-transform ${pullDistance >= 60 ? 'rotate-180' : ''}`} />
                  <span className='font-medium'>{pullDistance >= 60 ? t('松手刷新') : t('下拉刷新')}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className='content relative flex-1 flex flex-col overflow-hidden'>
          ...
          <div
            ...
            className={`flex-1 overflow-y-auto ${getAppId() === 'janur' ? 'bg-[#F7F3EE]' : 'bg-[#f5f5f5]'} px-4 py-3 ${totalPages > 1 && !loading ? 'pb-[60px]' : ''}`}
          >
            {worksList.length === 0 && !loading ? (
              <div className='flex flex-col items-center justify-center h-64'>
                {getUid() ? (
                  ...
                  <>
                    <div className='mb-3 text-sm text-gray-400'>{t('登录后可查看你的作品')}</div>
                    <Button onClick={() => store.setLoginShow(true)} className='px-4 py-2 rounded-full'>
                      {t('去登录')}
                    </Button>
                  </>
                )}
                ...
                {worksList.map((work: SerializedWorksEntity) => {
                  return (
                    <WorkInfoCard
                      ...
                      toEdit={() => {
                        store.push(`/mobile/editor?works_id=${work.id}&uid=${getUid()}`);
                      }}
                    />
                  );
                })}
                  ...
                  <PaginationContent className='gap-2 w-full'>
                    ...
                    <div className='flex gap-2 items-center'>
                      {(() => {
                          ...
                          return Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                            <PaginationItem key={num}>
                              <PaginationLink
                                onClick={() => setPage(num)}
                                isActive={page === num}
                                className={`cursor-pointer h-8 w-8 text-xs bg-white border ${page === num ? 'border-blue-500 text-blue-600 font-semibold' : 'border-gray-300'
                                  }`}
                              >
                                {num}
                              </PaginationLink>
                        ...
                        for (let i = showStart; i <= showEnd; i++) pages.push(i); // 当前页附近
                          ...
                          return (
                            <PaginationItem key={item}>
                              <PaginationLink
                                ...
                                isActive={page === item}
                                className={`cursor-pointer h-8 w-8 text-xs bg-white border ${page === item ? 'border-blue-500 text-blue-600 font-semibold' : 'border-gray-300'
                                  }`}
                              >
                                {item}
                              </PaginationLink>
