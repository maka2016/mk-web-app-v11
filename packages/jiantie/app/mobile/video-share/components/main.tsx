...
const VideoExport = (props: Props) => {
  ...
  const checkStatus = async () => {
    ...
    if (res.data.status === 2) {
      ...
      onSave(res.data.oss_path, worksDetail?.title);
    }
        ...
        <div className={styles.iframeContainer}>
          <div className={styles.border} style={iframeStyle}>
            <iframe
              ...
              src={`${location.origin}/viewer2/${worksId}?appid=${appid}&exportVideo=1`}
            />
      ...
      {videoUrl && !isPC && (
        <div className={styles.footer}>
          ...
          <div className={styles.shareTypes}>
            {isMakaAppClient() && shareWechatVideo && (
              <BehaviorBox
                behavior={{
