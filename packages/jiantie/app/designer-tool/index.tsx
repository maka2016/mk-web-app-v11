...
import { getDesignerInfoForClient } from '@mk/services';
    ...
    <div className='p-4'>
      ...
      <Button
        onClick={() => {
          ...
          window.open(
            `/editor-designer?works_id=${templateId}&designer_tool=2&uid=${getUid()}&is_template=true`
          );
        }}
      >
        确定
...
const ChangeSpec = ({
  ...
  const handleCreateWorks = async (item: SpecItem) => {
    try {
      ...
      console.log(res);
      window.open(
        `/editor-designer?works_id=${res.id}&designer_tool=2&uid=${getUid()}&appid=${getAppId()}&works_cate=${worksCate}`
      );
    } catch (error: unknown) {
                ...
                {specList.map(item => (
                  <tr
                    ...
                    className={`cursor-pointer transition-colors hover:bg-gray-50 ${
                      selectedSpec?.id === item.id ? 'bg-blue-50' : ''
                    }`}
      ...
      <div className='flex justify-between'>
        <div className='p-4 flex flex-col gap-2'>
          ...
          <Button
            variant='outline'
            onClick={() => {
              setDefaultWorksName('设计师模版作品');
              setShowSpec(true);
              setWorksCate('template');
            }}
          >
            创建模版
          </Button>
          ...
          <Button
            ...
            onClick={() => {
              // setShowTemplate(true);
              window.open('/designer/works-manager');
            }}
          >
            设计师作品管理
          </Button>
