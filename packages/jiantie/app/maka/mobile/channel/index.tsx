...
import { isMakaAppAndroid, queryToObj, random } from '@/utils';
...
const Home = (props: {
  ...
  const hasCheckedDefaultChannel = useRef(false);
  ...
  const fetchFloors = async () => {
    ...
    if (res?.data?.rows) {
      ...
      if (floor) {
        ...
        const index = 0;
        const defaultHotwordMeta = floor.hot_word[index].hot_word_meta[0];

        replaceUrlArg(floor.hot_word[index].hot_word_tag, 'ref_page_id');
        replaceUrlArg(defaultHotwordMeta.hot_word, 'hotword_floor_word_btn');
        // 进入频道首页时，以默认 hotword 作为 pageview 的依据
        pageView(defaultHotwordMeta.hot_word);
  ...
  const toSearch = () => {
    ...
    if (APPBridge.judgeIsInApp()) {
      APPBridge.navToPage({
        url: `${location.origin}/maka/mobile/channel/search-mix?parent_page_type=${parent_page_type}&ref_page_id=${query.ref_page_id
      ...
      location.href = `/maka/mobile/channel/search-mix?parent_page_type=${parent_page_type}&ref_page_id=${query.ref_page_id
        }&page_inst_id=${decodeURIComponent(query.page_inst_id || '')}&hotword_floor_word_btn=${decodeURIComponent(
          query.hotword_floor_word_btn || ''
        )}`;
    }
  ...
  const toChannel = (item: Resource) => {
    ...
    if (APPBridge.judgeIsInApp()) {
      ...
      window.location.href = `/maka/mobile/channel/topic?id=${alias}`;
    }
  };
  ...
  return (
    <TrackingContext.Provider value={trackingValue}>
      <div
        className='flex flex-col bg-[length:100%_240px] bg-no-repeat bg-white h-full overflow-y-auto overscroll-none touch-pan-y'
        ref={scrollRef}
        style={{
          backgroundImage,
        }}
      >
        <div
          className='bg-white pt-[var(--safe-area-inset-top)] bg-[length:100%_240px] sticky top-0 z-[2]'
          style={{
            backgroundImage,
          }}
        ></div>
      ...
      // 暂时关闭测试入口
        ...
        {entrances.length > 0 && (
          <div
            className='flex-shrink-0 sticky top-[var(--safe-area-inset-top)] bg-[length:100%_180px] bg-no-repeat w-full overflow-x-auto overflow-y-hidden z-[9]'
            style={{
              backgroundImage,
            }}
          >
