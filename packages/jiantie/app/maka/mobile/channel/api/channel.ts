...
/** 素材楼层 */
export const getMaterialFloors = (params: Record<string, any>) => {
  return request.get(`${API('MkServer')}/store-v1/v2/site/floor`, {
    params,
  });
};

/** 获取素材列表 */
export const getMaterialsByFilterId = (params: Record<string, any>) => {
  return request.get(
    `${API('MkServer')}/content-center-api-v1/filter/materials`,
    {
      params,
    }
  );
