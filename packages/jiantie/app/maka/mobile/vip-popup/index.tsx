...
import { isAndroid, isMakaAppIOS, isWechat, queryToObj } from '@mk/utils';
...
const Payment = (props: Props) => {
  ...
  const [activeTab, setActiveTab] = useState(
    works_type === 'h5' ? 'full_site_vips' : 'post_vips'
  );
  ...
  const toShare = async () => {
    ...
    if (works_type === 'h5') {
      ...
      if (APPBridge.judgeIsInApp()) {
        APPBridge.navToPage({
          url: `${location.origin}/maka/mobile/download?works_id=${works_id}&uid=${uid}&is_full_screen=1`,
          type: 'URL',
        });
      } else {
        location.href = `/maka/mobile/download?works_id=${works_id}&uid=${uid}&appid=${getAppId()}`;
      }
    }
  };
  ...
  return (
    <div className={cls([styles.top, styles[topClassName()]])}>
      ...
      <div
        className={cls([styles.tabWrap, works_type === 'h5' && styles.single])}
      >
        {works_type !== 'h5' ? (
