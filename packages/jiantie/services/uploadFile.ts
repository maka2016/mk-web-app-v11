...
/** v11 独立实现：按 type 直传 OSS，返回与 FileItem 兼容的结构 */
async function uploadFileViaOss(params: UploadFileParams): Promise<FileItem> {
  await startupStsOssClient({
    appid: getAppId(),
    uid: getUid(),
    token: getToken(),
  });
  const type = params.type ?? 'picture';
  const ext = (/\.[^.]+$/.exec(params.file.name)?.[0] ?? '') || '.jpg';
  const fileName = `${Date.now()}${ext}`;
  const folder = type === 'music' ? 'musics' : type === 'video' ? 'videos' : 'timages';
  const ossKey = `${folder}/${fileName}`;
  const result = await uploadFileToOSS(params.file, ossKey);
  return {
    id: 0,
    appid: getAppId() ?? '',
    uid: Number(getUid()) || 0,
    ...
    ossKey: result.name,
    originName: params.originName ?? params.file.name,
    type,
...
export const uploadFile = async (params: UploadFileParams) => {
  ...
  if (useV11API) {
    return uploadFileViaOss(params);
  }
