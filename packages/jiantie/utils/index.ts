...
export function markdownToTemplate(markdown: string): TemplateGenConfig {
  const lines = markdown
    .split('\n')
    .map(line => line.trim())
    .filter(line => line);
  let currentSection = '';
  const template: TemplateGenConfig = {
    ids: [],
    btnName: '',
    promptStr: '结构：',
  };
  ...
  lines.forEach(line => {
    if (line.startsWith('# ')) {
      currentSection = line.slice(2).trim(); // Remove '# ' and get the section title
    } else {
      switch (currentSection) {
        case '模版id':
          template.ids.push(line);
          break;
        case '按钮名称':
          template.btnName = line;
          break;
        case '生成结构':
          template.promptStr += line + '\n'; // Keep appending lines to promptStr
          break;
      }
    }
  });

  template.promptStr = template.promptStr.trim(); // Remove extra whitespace at the end
  return template;
}
...
export const getENV = () => {
  if (process.env.ENV !== 'staging' && process.env.ENV !== 'prod') {
    return 'testing';
  } else {
