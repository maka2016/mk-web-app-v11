...
export async function tokenToUid(token: string): Promise<{
  ...
  const userCenterUrl =
    process.env.NODE_ENV === 'production'
      ? process.env.nest_user_center_k8s
      : process.env.nest_user_center_dev;

  if (!userCenterUrl) {
    console.error(
      'Missing environment variable: nest_user_center_k8s or nest_user_center_dev'
    );
...
export async function validateRequest(
  ...
  const isAppidMatch = requestedAppid
    ? String(requestedAppid) === String(tokenAppid)
    : true;
