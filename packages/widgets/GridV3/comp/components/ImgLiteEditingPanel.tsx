...
import { blockStyleFilter, removeOssParamRegex } from '../../shared/utils';
import { mergeDeep, queryToObj } from '@mk/utils';
import {
  API,
  cdnApi,
  getPageId,
...
const ImgLiteEditingPanel = (props: Props) => {
  ...
  const handleSave = async () => {
    if (onChange) {
      ...
      onChange({
        ...
        baseH: imageSize.originalWidth,
        baseW: imageSize.originalHeight,
  ...
  const recalculateTransform = async () => {
    ...
    if (backgroundRef.current) {
      backgroundRef.current.style.transform = `translate(${offsetLeft}px, ${offsetTop}px) scaleX(${
        flipHorizontal ? -1 : 1
      })`;
    }
  };
...
export default ImgLiteEditingPanel;
