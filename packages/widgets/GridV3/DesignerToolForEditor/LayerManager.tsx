  ...
  const getElementContent = () => {
    try {
      ...
      if (/text/gi.test(elementRef)) {
        ...
        const textContent = textAttrs.text || '文字内容';
        ...
        <div className='w-full'>
          ...
          {hasElements &&
            node.childrenIds!.map(elemId => {
              ...
              const layerLink = editorSDK?.getLink(elemId);
              const isDisabledToEdit = layer?.attrs?.disabledToEdit;
              const isAbsoluteElem = layer?.attrs?.absoluteElem;
              const layerTag = layerLink?.tag as any;
...
export default function LayerManager() {
  ...
  useEffect(() => {
    // 根据editingElemId找到父级路径并展开
    const currRow = getActiveRow() as any;
    const parentPath = currRow?.parentRowIds || [];
    autoExpandToPath(parentPath);
    if (editingElemId) {
      // 滚动到对应的元素
...
export const LayerManagerPopover = () => {
    ...
    <Popover open={open}>
      <PopoverTrigger asChild>
        <Button
          variant='outline'
          size='sm'
          className={`fixed bottom-[8px] right-[300px] z-[9999]`}
          title='快捷键说明'
      ...
      <PopoverContent side='top' align='end' className='p-0 w-[320px]'>
        <div className='header flex items-center justify-between pl-2'>
          <div className='title'>图层管理</div>
