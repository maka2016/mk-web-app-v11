  ...
  for (const shadow of shadows) {
    ...
    for (const part of parts) {
      if (part.match(/^(-?[\d.]+)px?$/)) {
        ...
        color = normalizeColor(part); // 提取并标准化颜色
      }
    }
      ...
      {shadows.length > 0 && (
        <div className='flex items-center gap-2'>
          ...
          <ColorPickerPopover
            ...
            onChange={value => {
              handleSetAllColors(value ? value.hex || value.value : '');
            }}
          />
        </div>
      )}
          ...
          <ColorPickerPopover
            ...
            onChange={value => {
              handleUpdateShadow(index, {
                color: value ? value.hex || value.value : undefined,
              });
            }}
          />
...
export { parseShadow, formatShadow, normalizeColor };
