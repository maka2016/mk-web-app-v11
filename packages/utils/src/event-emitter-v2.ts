 ...
 *
 * // 或者自定义一个全新的对象
 * const _EventEmitter = new EventEmitterClassV2();
 *
 * const callback = () => {};
 *
 ...
 */
export class EventEmitterClassV2 {
  ...
  on: Subscribe = (
    ...
    this.subscribe(eventName, func, execTime);
  };

  rm: Subscribe = (eventName: string, func: SubscriptFunction) => {
    this.unsubscribe(eventName, func);
  };

  once = (eventName: string, func: SubscriptFunction) => {
    this.subscribe(eventName, func, 1);
  };

  private checkFuncIsExist(eventName: string, func: SubscriptFunction) {
...
export const EventEmitterV2 = new EventEmitterClassV2();
